...
- **One figure.** Formation shapes for a second and third are a later slice; the
  slot arithmetic that would place them already exists, and so does the cap that
  will stop several of them talking over each other.
...
- **It cannot wear your own outfit.** The game exposes no single call that turns
  ...
  NPCs — that means a cache decoder, and it is the next slice.
