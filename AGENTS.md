...
- **The repo path contains a space** (`.../Mariner Digital Agency/Projects/osrs/entourage`).
...
- **A guard is proven by breaking it.** The discipline this project holds
  ...
  worse than no test, because it reads as coverage that is not there.
...
- Only read/write files inside the `.runelite` directory. Create a subdirectory for your plugin (e.g. `.runelite/entourage/`) if you need to store data on disk.
...
## Cosmetic-figure specifics for this plugin

Entourage draws `RuneLiteObject`s next to the local player. Nobody else's client
sees them, no packet is ever sent, and the plugin reads no information about any
other player, so the Boss & Combat, PvP and Data & Privacy sections above are not
expected to bear on it. Three rules that do:

- **Nothing this plugin draws may be mistaken for a real entity.** A figure that
  walks with you sits inside your click radius all day. The moment this plugin
  grows a right-click surface, those entries have to be deprioritised,
  `MenuAction.RUNELITE`-typed and carry an Examine that says out loud what the
  figure is — the treatment `CitizenMenu` in `../lively-cities` already works out.
  Until then it adds no menu entries at all, which is the strongest version of the
  same guarantee.
- **No copyrighted or trademarked likeness, name or design.** "Anime style" in the
  brief means a deliberate stance and a coordinated formation. It does not mean a
  character. Every model this plugin builds comes from the game's own cache
  through an `NPCComposition`.
- **`shutDown()` leaves zero registered objects.** A leaked `RuneLiteObject` is a
