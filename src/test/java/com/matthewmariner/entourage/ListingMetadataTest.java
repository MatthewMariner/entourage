 ...
 *
 * <p><b>This is not hypothetical.</b> A sibling plugin in this family, {@code
 * ../gunnars-tools}, shipped with its properties file and its annotation describing the
 * same feature in two different sentences — neither wrong, discovered on the submission
 * checklist rather than by anything that runs. Entourage's own pair was already
 * byte-identical when this test was written — see {@link EntouragePlugin}'s class
 * javadoc — but "correct right now" is exactly when a guard like this is cheap to add and
 * cheapest to keep green.
 *
...
public class ListingMetadataTest
{
	...
	private static PluginDescriptor descriptor()
	{
		PluginDescriptor descriptor =
			EntouragePlugin.class.getAnnotation(PluginDescriptor.class);
		assertNotNull("EntouragePlugin has lost its @PluginDescriptor", descriptor);
		return descriptor;
	}
	...
	{
		assertEquals(EntouragePlugin.class.getName(), listing().getProperty("plugins"));
	}

	/**
	 * The exact three placeholder values {@code runelite/example-plugin} ships its own
	 * {@code runelite-plugin.properties} with, fetched from that repository rather than
	 * guessed — {@code displayName=Example}, {@code author=Nobody},
	 * {@code description=An example greeter plugin}. Compared case-insensitively after
	 * trimming, because the hub's own reader would treat {@code "Example "} and
	 * {@code "EXAMPLE"} as the same unedited field a case-sensitive exact match would miss.
	 *
	 * <p><b>Why an exact match rather than the substring sweep this replaced.</b> The old
	 * check refused any value whose lowercase form <i>contained</i> {@code "example"},
	 * {@code "your name"} or {@code "todo"} — a heuristic standing in for a fact this class
	 * can simply know. It would eventually refuse a perfectly good description this plugin
	 * might actually write, such as "…for example, Vannaka" — a false positive on ordinary
	 * prose rather than on a forgotten placeholder. It never fired on any text this repo has
	 * shipped, so replacing it is pre-emptive rather than a fix for something broken today.
	 */
	private static final Map<String, String> TEMPLATE_PLACEHOLDERS = new LinkedHashMap<>();

	static
	{
		TEMPLATE_PLACEHOLDERS.put("displayName", "Example");
		TEMPLATE_PLACEHOLDERS.put("author", "Nobody");
		TEMPLATE_PLACEHOLDERS.put("description", "An example greeter plugin");
	}
	...
	public void noFieldTheHubRefusesToPackageIsLeftAtItsTemplateValue() throws IOException
	{
		...
		// `version` is the one field that is correctly left empty — the hub falls back to
		// the commit — so it is not among the placeholders checked here.
		for (Map.Entry<String, String> placeholder : TEMPLATE_PLACEHOLDERS.entrySet())
		{
			String field = placeholder.getKey();
			String value = listing.getProperty(field);
			assertNotNull(field + " is missing", value);
			assertFalse(field + " is empty", value.trim().isEmpty());
			assertFalse(field + " still holds the template's placeholder value",
				value.trim().equalsIgnoreCase(placeholder.getValue()));
		}
