 ...
 *
 * <p>Mechanically generated from {@code javap net.runelite.api.Player} and its supertypes,
 ...
 * 80, and so that a method this plugin has never used fails loudly the first
 * time a test reaches it instead of quietly returning {@code null}.
 *
 * <p>{@link FakePlayer} is the subclass that answers {@code getLocalLocation()}, which is the only thing {@link FollowerAnchor} asks a player for.
 *
...
class StubPlayer implements net.runelite.api.Player
{
	static UnsupportedOperationException unsupported(String method)
	{
		return new UnsupportedOperationException(
			"StubPlayer does not implement Player." + method + "(..) — this plugin has never needed it");
	}
