 ...
 *
 * <p>Mechanically generated from {@code javap net.runelite.api.Model} and its supertypes,
 ...
 * 58, and so that a method this plugin has never used fails loudly the first
 * time a test reaches it instead of quietly returning {@code null}.
 *
 * <p>Nothing reads a Model in these tests; it only has to exist so that {@code light()} can return something non-null.
 *
...
class StubModel implements net.runelite.api.Model
{
	static UnsupportedOperationException unsupported(String method)
	{
		return new UnsupportedOperationException(
			"StubModel does not implement Model." + method + "(..) — this plugin has never needed it");
	}
