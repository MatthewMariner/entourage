 ...
 *
 * <p>Mechanically generated from {@code javap net.runelite.api.ModelData} and its supertypes,
 ...
 * 34, and so that a method this plugin has never used fails loudly the first
 * time a test reaches it instead of quietly returning {@code null}.
 *
 * <p>{@link FakeModelData} is the subclass that records the recolour and lighting calls.
 *
...
class StubModelData implements net.runelite.api.ModelData
{
	static UnsupportedOperationException unsupported(String method)
	{
		return new UnsupportedOperationException(
			"StubModelData does not implement ModelData." + method + "(..) — this plugin has never needed it");
	}
